...
const showPrintPreview = ref(false) // Новое состояние для предпросмотра

// Состояния для редактирования полей товаров
const editingItem = ref(null)
const editingField = ref('')

// Данные заявки (все поля редактируемые)
const order = ref({
  requestNumber: generateRequestNumber(),
  requestDate: new Date().toLocaleDateString('ru-RU'),
  
  // Данные заявителя (из пропсов, но можно редактировать)
  ...
  reason: '', // Причина ремонта/нужды
  items: []
})

// Типы запчастей для чекбоксов (исправляем)
const partTypes = ref([
...
function addItem() {
  ...
  if (newItem.value.product) {
    ...
    // Если не нашли по артикулу, возможно, это товар без артикула
    if (!selectedProduct) {
      // Проверяем, может быть это индекс в массиве
  ...
  // Ищем остаток на складе в различных возможных полях
  let stock = 0;
  
  // Проверяем возможные названия полей с остатком
  ...
  // Создаем ID для товара
  const itemId = Date.now() + Math.random()
  
  // Получаем название товара из возможных полей
  ...
  order.value.items.push({
    ...
    stock: stock,
    // Сохраняем ссылку на оригинальный товар для возможного дальнейшего использования
...
// Функция для получения остатка из товара
...
// Новая функция для создания HTML документа
...
<template>
  <Teleport to="body">
    ...
    <div v-if="modelValue" class="modal-overlay2" @click.self="close">
      <div :class="['modal-content2', { 'print-mode': isPrintMode }]">
        <!-- Успешное оформление -->
