...
class InventoryRequest(Base):
    ...
    reason = Column(String(50), default="manual")#manual (по кнопке) | auto (автоматическая по минимальному запасу)
    status = Column(String(50), default="новая")  # новая|в_процессе|выполнена|отменен
